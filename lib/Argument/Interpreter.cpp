...
size_t DLDL::argument::Interpreter::Run()
{
	...
	if (!InitializeLanguages()) return -1;
...
void DLDL::argument::Interpreter::Help()
{
	std::cout
	<< 
		"This is the help page, this shows all the documented arguments that you can use with DLDL.\n"
...
void DLDL::argument::Interpreter::Initialize()
{
	if (parser.IsArgumentSet(Type::initialize))
	{
		...
		generate::Project(languages).WriteToDisk(rootDirectory);
	}
}
...
void DLDL::argument::Interpreter::About()
{
	...
	<<
		...
		"Using Deamer CC you can compile the definitions and generate your defined compiler and ecosystem.\n"
		"\n"
		"For more information about DLDL go to: https://github.com/Deruago/DLDL \n"
		"For more information about Deamer CC go to: https://github.com/Deruago/theDeamerProject \n"
		"\n"
	;
}
...
{
	projectGeneration = new DLDL::generate::Project(languages);
...
void DLDL::argument::Interpreter::GitOptions()
{
	if (parser.IsArgumentSet(Type::git_initialize))
	{
		...
		if (!gitDirectoryExists)
		{
			...
			const std::string gitInitialCommit =
				"git commit -m \"Initial Commit!\"";
			const deamer::file::tool::Action gitInitialCommitAction = { gitInitialCommit };
			std::system(gitInitialCommitAction.GetSubShellAction(deamer::file::tool::os_used).c_str());
		}
	}
}
