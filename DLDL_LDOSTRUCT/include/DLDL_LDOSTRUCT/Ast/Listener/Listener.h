#ifndef DLDL_LDOSTRUCT_AST_LISTENER_DEAMER_LISTENER_H
#define DLDL_LDOSTRUCT_AST_LISTENER_DEAMER_LISTENER_H

#include <Deamer/External/Cpp/Ast/Listener.h>
#include "DLDL_LDOSTRUCT/Ast/Node/DLDL_LDOSTRUCT.h"
#include "DLDL_LDOSTRUCT/Ast/Enum/Type.h"
#include "DLDL_LDOSTRUCT/Ast/Node/COLON.h"
#include "DLDL_LDOSTRUCT/Ast/Node/LEFT_ANGLE_BRACKET.h"
#include "DLDL_LDOSTRUCT/Ast/Node/RIGHT_ANGLE_BRACKET.h"
#include "DLDL_LDOSTRUCT/Ast/Node/VARNAME.h"
#include "DLDL_LDOSTRUCT/Ast/Node/VARNAME_EXT.h"
#include "DLDL_LDOSTRUCT/Ast/Node/INDENTED_VALUE.h"
#include "DLDL_LDOSTRUCT/Ast/Node/ESCAPE_CHARS.h"

#include "DLDL_LDOSTRUCT/Ast/Node/program.h"
#include "DLDL_LDOSTRUCT/Ast/Node/deamerreserved_star__stmt__.h"
#include "DLDL_LDOSTRUCT/Ast/Node/stmt.h"
#include "DLDL_LDOSTRUCT/Ast/Node/argument.h"
#include "DLDL_LDOSTRUCT/Ast/Node/argument_name.h"
#include "DLDL_LDOSTRUCT/Ast/Node/argument_block.h"
#include "DLDL_LDOSTRUCT/Ast/Node/deamerreserved_star__deamerreserved_or__INDENTED_VALUE__deamerreserved_or__VARNAME_EXT__VARNAME______.h"

namespace DLDL_LDOSTRUCT { namespace ast { namespace listener { 

	class Listener : public ::deamer::external::cpp::ast::Listener
	{
	private:
	public:
		Listener() = default;
		~Listener() override = default;
	public:
		void Dispatch(const ::deamer::external::cpp::ast::Node* node) override
		{
			const auto enumeratedValue = static_cast<DLDL_LDOSTRUCT::ast::Type>(node->GetType());
			switch(enumeratedValue)
			{
			case DLDL_LDOSTRUCT::ast::Type::COLON:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::COLON*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::LEFT_ANGLE_BRACKET:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::LEFT_ANGLE_BRACKET*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::RIGHT_ANGLE_BRACKET:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::RIGHT_ANGLE_BRACKET*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::VARNAME:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::VARNAME*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::VARNAME_EXT:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::VARNAME_EXT*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::INDENTED_VALUE:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::INDENTED_VALUE*>(node));
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::ESCAPE_CHARS:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::ESCAPE_CHARS*>(node));
				break;
			}

			case DLDL_LDOSTRUCT::ast::Type::program:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::program*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::deamerreserved_star__stmt__:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::deamerreserved_star__stmt__*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::stmt:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::stmt*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::argument:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::argument*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::argument_name:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::argument_name*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::argument_block:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::argument_block*>(node));
				DefaultAction(node);
				break;
			}
			case DLDL_LDOSTRUCT::ast::Type::deamerreserved_star__deamerreserved_or__INDENTED_VALUE__deamerreserved_or__VARNAME_EXT__VARNAME______:
			{
				Listen(static_cast<const DLDL_LDOSTRUCT::ast::node::deamerreserved_star__deamerreserved_or__INDENTED_VALUE__deamerreserved_or__VARNAME_EXT__VARNAME______*>(node));
				DefaultAction(node);
				break;
			}
			}
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::COLON* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::LEFT_ANGLE_BRACKET* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::RIGHT_ANGLE_BRACKET* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::VARNAME* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::VARNAME_EXT* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::INDENTED_VALUE* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::ESCAPE_CHARS* node)
		{
		}

		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::program* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::deamerreserved_star__stmt__* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::stmt* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::argument* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::argument_name* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::argument_block* node)
		{
		}
		virtual void Listen(const DLDL_LDOSTRUCT::ast::node::deamerreserved_star__deamerreserved_or__INDENTED_VALUE__deamerreserved_or__VARNAME_EXT__VARNAME______* node)
		{
		}
	private:
		void DefaultAction(const ::deamer::external::cpp::ast::Node* node)
		{
			for(const auto* child : node->GetNodes())
			{
				Dispatch(child);
			}
		}
	};

}}}

#endif // DLDL_LDOSTRUCT_AST_LISTENER_DEAMER_LISTENER_H
