...
#include "DLDL_PRECEDENCE/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_PRECEDENCE::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_PRECEDENCE::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_PRECEDENCE::ast::relation::ConvertEnumToText(static_cast<DLDL_PRECEDENCE::ast::Type>(token->Type)) << "\n";
