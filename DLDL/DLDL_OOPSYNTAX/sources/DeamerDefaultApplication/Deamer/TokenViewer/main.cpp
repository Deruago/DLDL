...
#include "DLDL_OOPSYNTAX/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_OOPSYNTAX::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_OOPSYNTAX::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_OOPSYNTAX::ast::relation::ConvertEnumToText(static_cast<DLDL_OOPSYNTAX::ast::Type>(token->Type)) << "\n";
