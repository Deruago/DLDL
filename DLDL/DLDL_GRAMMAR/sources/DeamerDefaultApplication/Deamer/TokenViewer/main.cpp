...
#include "DLDL_GRAMMAR/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_GRAMMAR::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_GRAMMAR::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_GRAMMAR::ast::relation::ConvertEnumToText(static_cast<DLDL_GRAMMAR::ast::Type>(token->Type)) << "\n";
