...
#include "DLDL_LPDSTRUCT/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_LPDSTRUCT::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_LPDSTRUCT::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_LPDSTRUCT::ast::relation::ConvertEnumToText(static_cast<DLDL_LPDSTRUCT::ast::Type>(token->Type)) << "\n";
