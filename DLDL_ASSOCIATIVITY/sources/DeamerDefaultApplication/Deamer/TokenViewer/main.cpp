...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_ASSOCIATIVITY::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_ASSOCIATIVITY::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_ASSOCIATIVITY::ast::relation::ConvertEnumToText(static_cast<DLDL_ASSOCIATIVITY::ast::Type>(token->Type)) << "\n";
