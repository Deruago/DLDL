...
#include "DLDL_IDENTITY/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_IDENTITY::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_IDENTITY::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_IDENTITY::ast::relation::ConvertEnumToText(static_cast<DLDL_IDENTITY::ast::Type>(token->Type)) << "\n";
