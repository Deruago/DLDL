...
DLDL::OutputFormatter::OutputFormatter(const DLDL::DLDL_AstVisitor& ast_visitor, const std::string& language_name, const std::vector<DLDL::Argument>& arguments)
: Arguments(arguments)
{
...
{
	if (IsArgumentActivated(DLDL::ArgumentType_t::addThreatAnalysis))
	{
		return
...
std::string DLDL::OutputFormatter::PrintLanguageDefinition() const
{
	if (IsArgumentActivated(DLDL::ArgumentType_t::showstructure))
	{
		return "    LanguageDefinitionPrinter().Print(*language_definition);\n";
	}
	else
	{
		return "";
...
std::string DLDL::OutputFormatter::MakeMakeFileWithDebugMessages() const
{
	return
		"LANGUAGE=" + LanguageName + "\n"
		"LANG_LEXER=$(LANGUAGE)_Lexer.o\n"
...
bool DLDL::OutputFormatter::IsArgumentActivated(const DLDL::ArgumentType_t type) const
{
