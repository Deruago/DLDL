...
#include "DLDL_GRAMMAR_PRODUCTION_RULE/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_GRAMMAR_PRODUCTION_RULE::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_GRAMMAR_PRODUCTION_RULE::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_GRAMMAR_PRODUCTION_RULE::ast::relation::ConvertEnumToText(static_cast<DLDL_GRAMMAR_PRODUCTION_RULE::ast::Type>(token->Type)) << "\n";
