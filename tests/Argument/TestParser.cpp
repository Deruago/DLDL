...
#define ASSERT_VALUE_FROM_ARGUMENT(value, argument_)            \
	{                                                           \
		DLDL::argument::Parser parser;                          \
		ASSERT_EQ(value, parser.GetValueFromString(argument_)); \
	}
#define ASSERT_ENUM_VALUE_FROM_ARG(type, argument_)               \
	{                                                             \
		DLDL::argument::Parser parser = DLDL::argument::Parser(); \
		ASSERT_EQ(type, parser.GetTypeFromString(argument_));     \
	}
...
TEST_F(TestParser, OverwriteArguments_CorrectlyOverwritesArguments)
{
	parser = DLDL::argument::Parser(std::vector<std::string>{"-info", "-lpd-name=Lexicon"});
	parser.Overwrites("-lpd-map=\"lpd_definition\"");

	EXPECT_TRUE(parser.IsArgumentSet(DLDL::argument::Type::information));
	EXPECT_TRUE(parser.IsArgumentSet(DLDL::argument::Type::lpd_name));
	EXPECT_TRUE(parser.IsArgumentSet(DLDL::argument::Type::lpd_map));

	EXPECT_EQ("lpd_definition", parser.GetArgument(DLDL::argument::Type::lpd_map).value);
}
