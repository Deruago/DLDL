...
#include "DLDL_LPDDEF_EF/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = DLDL_LPDDEF_EF::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		DLDL_LPDDEF_EF::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << DLDL_LPDDEF_EF::ast::relation::ConvertEnumToText(static_cast<DLDL_LPDDEF_EF::ast::Type>(token->Type)) << "\n";
